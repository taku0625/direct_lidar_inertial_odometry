...
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle)
{
	...
	this->lidar_sub = this->nh.subscribe("pointcloud", 10,
...
void dlio::OdomNode::deskewPointcloud()
{
	...
	if (true)
	{
		...
		this->unique_time_indices.clear();

		this->deskewed_scan_->points.resize(this->original_scan->points.size());
		...
		// copy points into this->deskewed_scan_ in order of timestamp
		std::partial_sort_copy(this->original_scan->points.begin(), this->original_scan->points.end(),
							   this->deskewed_scan_->points.begin(), this->deskewed_scan_->points.end(), point_time_cmp);

		// filter unique timestamps
		auto points_unique_timestamps = this->deskewed_scan_->points | boost::adaptors::indexed() | boost::adaptors::adjacent_filtered(point_time_neq);
		...
		{
			this->timestamps.push_back(extract_point_time(*it));
			this->unique_time_indices.push_back(it->index());
		}
		this->unique_time_indices.push_back(this->deskewed_scan_->points.size());
	}

	int median_pt_index = this->timestamps.size() / 2;
	this->scan_stamp = this->timestamps[median_pt_index]; // set this->scan_stamp to the timestamp of the median point
	...
	if (!this->first_valid_scan)
	{
		...
		this->first_valid_scan = true;
		this->T_prior = this->T; // assume no motion for the first scan
		pcl::transformPointCloud(*this->deskewed_scan_, *this->deskewed_scan_, this->T_prior * this->extrinsics.baselink2lidar_T);
		*p = *this->deskewed_scan_;
		this->deskewed_scan = p;
	...
	this->deskew_size = frames.size(); // if integration successful, equal to this->timestamps.size()

	// if there are no frames between the start and end of the sweep
	// that probably means that there's a sync issue
	if (frames.size() != this->timestamps.size())
	{
...
#pragma omp parallel for num_threads(this->num_threads_)
	for (int i = 0; i < this->timestamps.size(); i++)
	{
		...
		for (int k = this->unique_time_indices[i]; k < this->unique_time_indices[i + 1]; k++)
		{
			auto &pt = this->deskewed_scan_->points[k];
									 ...
									 boost::circular_buffer<ImuMeas>::reverse_iterator end_imu_it)
{
	...
	for (; imu_it != end_imu_it; imu_it++)
	{
		...
		Eigen::Vector3f alpha_dt = (f.ang_vel - this->iterative_dbias.gyro * dt * i) - f0.ang_vel;
		Eigen::Vector3f alpha = alpha_dt / dt;
...
void dlio::OdomNode::updateState()
{
	...
	{
		// Update accel bias
		bias.accel = this->state.b.accel - dt * this->geo_Kab_ * err_body;
		bias.accel = bias.accel.array().min(abias_max).max(-abias_max);

		// Update gyro bias
		bias.gyro[0] = this->state.b.gyro[0] - dt * this->geo_Kgb_ * qe.w() * qe.x();
		bias.gyro[1] = this->state.b.gyro[1] - dt * this->geo_Kgb_ * qe.w() * qe.y();
		bias.gyro[2] = this->state.b.gyro[2] - dt * this->geo_Kgb_ * qe.w() * qe.z();
